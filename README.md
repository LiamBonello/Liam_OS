...
`make x86_64-run` routes COM1 serial output to the terminal. Expected x86_64 serial markers include `IRQ gates installed: 16`, `IRQ gates ok: 1`, `IRQ legacy PIC remapped: 1`, `IRQ all masked: 1`, `IRQ policy ok: 1`, `Runtime entry ok: 1`, and `Desc/IST ok: 1`.
...
Expected serial markers include `Exception self-test requested: 1`, `Name: invalid opcode`, `Vector: 0x0000000000000006`, and `x86_64 panic halt`.
