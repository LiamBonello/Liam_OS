...
It can also build and run an experimental Multiboot2 ISO that enters long mode, calls a freestanding x86_64 C entry, installs an early exception IDT, parses boot information, and writes VGA/serial diagnostics:
...
Stage: IDT exceptions + boot memory summary
Multiboot2: ok
Bootloader: ...
...
This is not the full x86_64 kernel yet. The current path proves an assembly handoff into long mode, a minimal C entry, early boot diagnostics, Multiboot2 tag parsing, and an early CPU exception IDT. It does not initialize IRQ routing, APIC/PIC/PIT, the shared paging subsystem, heap, processes, syscalls, or userspace.
